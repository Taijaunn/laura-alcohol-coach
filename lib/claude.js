...
/**
 * Extract just the first name from a user's message.
 * e.g. "My name is Taijaun" → "Taijaun"
 */
async function extractName(text) {
  const response = await client.messages.create({
    ...
    max_tokens: 50,
    system:
      "Extract ONLY the person's first name from their message. Return just the name, nothing else. Capitalize it properly.",
...
/**
 * Check if a user's reply is a genuine answer to an onboarding question.
 */
async function isValidOnboardingAnswer(question, answer) {
  const response = await client.messages.create({
    model: MODEL,
    max_tokens: 10,
    system:
      "You determine if a user's message is a genuine answer to a question. Reply ONLY 'yes' or 'no'.",
    messages: [
      {
        role: "user",
  ...
  return response.content[0].text.trim().toLowerCase() === "yes";
}
